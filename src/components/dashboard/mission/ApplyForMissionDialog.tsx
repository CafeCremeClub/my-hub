...
const ApplyForMissionDialog = ({
  ...
  const { isPending, mutateAsync } = useApplyForMission();
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-lg overflow-hidden rounded-[1.25rem]">
        <div className="bg-white flex flex-col gap-10 overflow-y-scroll scroll-hidden">
          ...
          {step === 1 ? (
            <form
              className="mt-4 px-2 flex flex-col gap-8 flex-1"
              onSubmit={formik.handleSubmit}
            >
              <div className="flex flex-col gap-1">
                <div className="flex-none flex justify-center items-center rounded-full size-12 bg-[#DCFAE6]">
                ...
                <p className="text-sm text-[#475467]">
                  Tu es sur le point de candidater à cette mission. Une fois
                  envoyé, ton profil sera directement visible par le recruteur.
                </p>
              </div>
              <div className="flex flex-col gap-1.5">
                ...
                {formik.touched.note && formik.errors.note && (
                  <CustomErrorIndicator message={formik.errors.note} />
                )}
              </div>
              {errorMessage && <CustomErrorIndicator message={errorMessage} />}
            ...
            <div className="mt-4 px-2 flex flex-col gap-8 flex-1">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-sm text-[#475467]">
                  Ta candidature a bien été envoyée. Le recruteur peut
                  maintenant consulter ton profil et te contacter si ton profil
                  correspond.
                </p>
              </div>

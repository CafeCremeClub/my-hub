...
export const getMissions = async (
  payload: GetMissionsPayload
): Promise<GetMissionsResponse> => {
  try {
    const response = await axiosInstance.post<GetMissionsApiResponse>(
      '/missions/all',
      payload
    );
    return {
...
export const applyForMission = async (
  payload: ApplyForMissionPayload
): Promise<void> => {
  try {
